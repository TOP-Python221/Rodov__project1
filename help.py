...
H_RULES = f"""Правила игры
    Вы играете одним из двух символов: крестиком {config.TOKENS[0]!r} или ноликом {config.TOKENS[1]!r}. Чтобы победить, первым составьте последовательность из {config.DIM} своих символов в одной строке, в одном столбце, либо в одной диагонали.
"""
BOARD_COORDINATES = [[f'{i+1} {j+1}' for j in config.RANGE] for i in config.RANGE]
...
if __name__ == '__main__':
    show_title('крестики-нолики', center=False, uppercase=False)
    show_title('крестики-нолики', uppercase=False)
    show_title('крестики-нолики')
    show_title('крестики-нолики', padding_vertical=True)
